<?php
    ...
    $TotalRecords = mysqli_num_rows( MysqlQuery("SELECT CartID FROM customer_shopping_cart") );

    $ShowTotalPages = $TotalRecords > $PerPage ? true : false;
    $navs = CreateNavs($TotalRecords, $PerPage, 'page', 9, 'Total Records - ', $ShowTotalPages);

    // GET PRODUCTS CUSTOMIZED FOR CUSTOMER
    $CustomerProducts = MysqlQuery("SELECT c.FirstName, c.LastName, sc.* FROM customer_shopping_cart sc LEFT JOIN customers c ON c.MemberID = sc.CustomerID ORDER BY sc.CartID DESC LIMIT ".$page.", ".$PerPage);
    $CustomerProducts = MysqlFetchAll($CustomerProducts);

    $SelectedPage = 'Customer Abandoned Cart';
