<?php
    ...
    if(isset($_GET['term']))
    {
        ...
        $query = "SELECT MemberID, FirstName, LastName FROM customers WHERE FirstName LIKE '%".$_GET['term'][0]."%' OR LastName LIKE '%".$_GET['term'][0]."%'";
        ...
        if(mysqli_num_rows($res) > 0)
        {
            for( ; $row = mysqli_fetch_assoc($res); )
            {
                ...
                {
                    $MatchesFound = 1000;
                    $results[$Name.'~'.$row['MemberID']] = $MatchesFound - strlen($MatchedText);
                }
                else
                {
                    $results[$Name.'~'.$row['MemberID']] = $MatchesFound;
                }
            }
        }
