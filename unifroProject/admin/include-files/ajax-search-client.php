<?php
    ...
    if(isset($_GET['term']))
    {
        ...
        $query = "SELECT ClientID, ClientName FROM clients WHERE ClientName LIKE '%".$_GET['term'][0]."%'";
        ...
        if(mysqli_num_rows($res) > 0)
        {
            for( ; $row = mysqli_fetch_assoc($res); )
            {
                ...
                {
                    $MatchesFound = 1000;
                    $results[$Name.'~'.$row['ClientID']] = $MatchesFound - strlen($MatchedText);
                }
                else
                {
                    $results[$Name.'~'.$row['ClientID']] = $MatchesFound;
                }
            }
        }
