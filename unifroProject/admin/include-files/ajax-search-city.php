<?php
    ...
    if(isset($_GET['term']))
    {
        ...
        $query = "SELECT CityID, CityName FROM cities WHERE CityName LIKE '%".$_GET['term'][0]."%'";
        ...
        if(mysqli_num_rows($res) > 0)
        {
            for( ; $row = mysqli_fetch_assoc($res); )
            {
                ...
                {
                    $MatchesFound = 1000;
                    $results[$Name.'~'.$Name] = $MatchesFound - strlen($MatchedText);
                }
                else
                {
                    $results[$Name.'~'.$Name] = $MatchesFound;
                }
            }
        }
