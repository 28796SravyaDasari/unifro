<?php
    ...
    if($EditMode)
    {
        $ClientAddresses = MysqlQuery("SELECT a.*, s.Name, c.CityName FROM client_delivery_addresses a LEFT JOIN cities c ON c.CityID = a.City LEFT JOIN states s ON s.StateID = a.State WHERE a.AddressID = '".$_GET['id']."' AND a.ClientID = '".$ClientID."' LIMIT 1");
        if(mysqli_num_rows($ClientAddresses) > 0)
        {
            $_POST = mysqli_fetch_assoc($ClientAddresses);
        }
    }
...
<html>
    <head>
        ...
        $(document).ready(function()
        {
            ...
            $("#AddClientAddress").submit(function(e)
            {
                e.preventDefault();
                ShowProcessing();
                    ...
                    <div class="row">
                        <div class="col-md-12" style="max-width: 700px">
                            <div class="card-box">
                                ...
                                <form action="/ajax/sales/add-address/" id="AddClientAddress">
                                    <input type="hidden" name="ClientID" value="<?=$ClientID?>" />
                                    <input type="hidden" name="AddressID" value="<?=$_GET['id']?>" />
                                    ...
                                    <div class="table-content">
                                        ...
                                        <div class="mg-t-30 pd-b-30">
                                            ...
                                            <a href="/sales/clients/addresses/<?=$ClientID?>/" class="btn btn-danger">Cancel</a>
                                        </div>
                                    </div>
                                </form>
                            </div>
