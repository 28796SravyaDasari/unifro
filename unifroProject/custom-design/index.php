<?php
    ...
    if($EditMode)
    {
        ...
        $fabricIDS = GetAllFabricsFromCustomData($JsonData);
        ...
        {
            if(!isset($FabricsList[$id]))
            {
                $FabricData = GetFabricDetails($id);
                $FabricsList[$id] = $FabricData;
            }
        }

        //$DefaultStyles = array($JsonData['Selections']['Styles']);
        $RedesignDefaults = $JsonData['Selections']['Styles'];
        $DefaultFabric['FabricID'] = $JsonData['Selections']['FabricID'];

        foreach($DefaultStyles as $key => $value)
        {
            $heading = array_keys($value)[0];
            if(!isset($RedesignDefaults[$heading]))
            {
                $RedesignDefaults = array_merge($RedesignDefaults,$value);
            }
        }

        $DefaultStyles = array($RedesignDefaults);
    }

    $DefaultFabricSec['FabricID'] = isset($DefaultFabricSec['FabricID']) ? $DefaultFabricSec['FabricID'] : '';
...
<html>
    <head>
        ...
        <style>
            ...
            [id^=Collar-1-Button]{ display: none; }
            [id^=Collar-1-Button-02]{ display: block; }
        </style>
        ...
        $(document).ready(function()
        {
            ...
            product.LoadDefaults({
                ...
                FabricIDSec: '<?=$DefaultFabricSec['FabricID']?>',
            ...
            //$('svg').find("#Epaulette image").attr("xlink:href", '/images/fabric-swatches/dn_13.jpg');
            //$('svg').find("#Epaulette").css({ display: "block" })
            //alert($('svg').find("#Epaulette").attr("style"))
            //$('#Collar-1-Button-02').show();
        ...
        <div class="content-page">
            ...
            <div class="content">
                ...
                <div class="svg-container">
                    <button class="btn btn-white print-btn" onclick="window.print()">Save Image</button>
                    <ul></ul>
