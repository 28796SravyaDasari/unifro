<?php
    ...
    if(VerifyFormToken('LoginForm'))
    {
        ...
        else
        {
            ...
            $MemberDetails = MysqlQuery("SELECT EmailID, Password, Status FROM customers WHERE EmailID = '".$_POST['EmailID']."' LIMIT 1");

            if(mysqli_num_rows($MemberDetails) == 1)
			{
				$MemberDetails = mysqli_fetch_assoc($MemberDetails);

				if($_POST['EmailID'] == $MemberDetails['EmailID'] && $MemberDetails['Status'] != '1')
				{
				    echo json_encode(array('status' => 'error', 'message' => 'Your account is blocked'));
				}
				...
				else
				{
                    ...
                    $q = MysqlQuery("UPDATE customers SET SID = '".$SID."', LastLoginTimestamp = CurrentLoginTimestamp, LastLoginIP = CurrentLoginIP, CurrentLoginTimestamp = ".time().", CurrentLoginIP = '".GetIP()."' WHERE EmailID = '".$_POST['EmailID']."' LIMIT 1");

					if(MysqlAffectedRows() == 1)
					{
					    $q = MysqlQuery("SELECT MemberID FROM customers WHERE SID = '".$SID."' LIMIT 1");
						$MemberDetails = mysqli_fetch_assoc($q);
						$MemberID = $MemberDetails['MemberID'];

						setcookie('sid', $SID, 0, '/');
                        RecordMemberActivity('Logged In', 'customers', $MemberID);

						if(isset($_SESSION['ReturnURL']))
						{
							if($_SESSION['ReturnURL'] == '/' || $_SESSION['ReturnURL'] == '/index.php')
							{
								$targetURL = $MasterMemberTypes[2]['MyAccountURL'];
							}
							else
